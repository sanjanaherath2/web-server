...
class WebServer(object):
    ...
    def __init__(web, port=10000):
        web.host = "localhost"  #socket.gethostname().split('.')[0] # Default to any avialable network interface
        web.port = port
        web.content_dir = 'html' # Directory where webpage files are stored
    ...
    def start():
        ...
        try:
            print("Starting your server on {host}:{port}".format(host=web.host, port=web.port))
            web_socket.bind((web.host, web.port))
            print("your Server started on port {port}.".format(port=web.port))

        except Exception as e:
            print("Error: Could not bind to port {port}".format(port=web.port))
            self.shutdown()
            sys.exit(1)
            ...
            if request_method == "GET" or request_method == "HEAD":
                ...
                try:
                    ...
                    if request_method == "GET": # Temporary 404 Response Page
                        response_data = b"<html><body><center><h1>Error 404: File not found</h1></center><p>Please Check The URL</p></body></html>"
